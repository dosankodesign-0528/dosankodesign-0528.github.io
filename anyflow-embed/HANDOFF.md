...
- **ピン留めセクション** (`.pin` > `.pin-vp`(sticky 100vh) > `.pin-stage`(1440×921を`--sp`でスケール))
  ...
  - `#cases` … 導入事例。カード3枚が順に**強ブラー(20px)**で出現 → 縦に並ぶ →
    スクロールで**上のカードの軸へ重なる**（後のカードほど `step` px ずつ下、既定24px。
    重なった3枚は画面の上下中央）。重なりきると**実寸へ拡大**して**3枚とも880px幅**・
    右余白120pxになる。**奥のカードにブラー/半透明は掛けない**
    - ⚠️ 絵コンテ最終コマは「奥ほど小さく大きくずれる」形（上端0/137.9/241.1・0.8/0.9/1.0）
      だったが、2026-08-14 にヒデさんの指示で**同じ軸に重ねる／幅は880px固定**に変更。
      絵コンテ形に戻したい時は調整パネルで 段差=120px前後 / 奥行き=100% にする
...
| 奥行きの強さ | `cases.depth` | **0%** | 1段につき縮小 10%。0だと3枚とも880px幅のまま |
| 重なりの段差 | `cases.step` | **24px** | 後のカードを下にずらす量。**マイナスにすると上にずれる**（奥のカードが下から覗く） |
