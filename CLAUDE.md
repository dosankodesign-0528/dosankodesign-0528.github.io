...
## 🧭 手戻り防止ルール（2026-08-15 制定・全プロジェクト）

> 📄 経緯と根拠は [docs/anyflow-postmortem.md](docs/anyflow-postmortem.md)。
> anyflow-embed で「カンプを渡したのに違うものが出る」「同じ不具合が3〜4回出る」が
> 実際に起きた。原因は注意不足ではなく**手順が無かったこと**だったので、手順として置く。

### ルール1: 着手前に「決まっていない表」を出す 🔴最重要

実装を始める前に、必ずこの表を出してから手を動かす。
...
**カンプは「止まった絵」しか決めていない。** 動き・間合い・順番・hover・画面幅を変えた時は
どの資料にも書かれていない。そこを黙って埋めるのが「勝手にデザインした」の正体。
**右列だけ先に見てもらえば、手戻りの9割は着手前に潰せる。**

### ルール2: 動き・間合いは言葉で合意しない。触れるものを出す

秒数・イージング・余白は文章で読んでも判断できない。
- 案が複数ある時 → mock を並べて URL で渡す（🎨 UI 提案ルール）
- 数値の詰め → **[tune-panel](tune-panel/) を先に入れて、ヒデさん本人が触って決める**

### ルール3: 「確認しました」は実測とセットでしか言わない

目視で書体・字間・余白は判別できない（実際に SF Pro Text/Display を取り違えて5%ズレた）。
`getComputedStyle` / `getBoundingClientRect` / `canvas.measureText` で測り、表で報告する。
スクショが撮れない箇所は「撮れないので実測で確認した」と明記する。

### ルール4: 同じ症状が2回出たら、対処をやめて原因を探す

2回目の修正は「たぶんこれだろう」で当てにいっている状態。**3回目に行く前に一段掘る。**
（anyflow-embed では「真っ白」3回・「切り替わりの違和感」4回やった）

### ルール5: CSS / JS を範囲でまとめて置換しない

- 置換の開始・終了マーカーは**同じセクション内**で取る。セクションをまたぐ範囲指定は使わない
- 消す前に、消える範囲の**末尾5行**を必ず目で見る（巻き添え削除を3回やっている）

### ルール6: 1ファイルが4,000行を超えたら分割を提案する

巻き添え削除は「注意すれば防げる」類ではなく、構造が呼び込んでいる。
