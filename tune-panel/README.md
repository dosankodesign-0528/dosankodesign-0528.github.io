...
## 🗣 用語の統一（パネルの言葉づかい・2026-08-23 ヒデさん制定）

**依頼時の言い回し（「スクロール深度」「尺」「タイミング」など）に合わせて毎回言葉を変えない。**
パネルに出す言葉は、概念ごとに下の1語へ正規化する。既存の項目と違う言葉を使いそうになったら、
まず既存のパネルを grep して同じ概念の項目名に合わせること。

| 概念 | 使う言葉 | 単位 | 使わない言い方の例 |
|---|---|---|---|
| スクロール量 | **「〜までの距離」** | px | 深度・尺・深さ・スクロール量 |
| 時間の待ち | **「〜までの間」** | ms / s | ディレイ・遅延・タイミング |
| 持続時間 | **「〜の長さ」「〜が出る時間」** | ms / s | 期間・尺 |
| 速度 | **「〜の速さ」** | s（1周など） | スピード |
| 強度 | **「〜の強さ」** | % / px | 量・強度・レベル |
| 出現 | **「登場」** | — | 出方・出現・表示（案ピルは「登場の案」） |
| 位置 | **「横ずれ／縦ずれ」「左からの距離」** | px | オフセット・XY |
| 大きさ | **「大きさ」** | px / % | サイズ・スケール |
