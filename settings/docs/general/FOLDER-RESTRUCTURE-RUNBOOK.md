...
> ✅ **2026-08-23 実施済み**：abashiri（site→`abashiri/v1`・site-v2→`abashiri/v2`）と anyflow（embed→`anyflow/v1`・embed-v2→`anyflow/v2`）を移動完了。
> abashiri-site は事前に **Git 連携を解除（手動化）** したので、手順Aの「Root Directory を先に直す」は不要だった（自動デプロイの地雷が先に消えていたため）。
> 以下は今後もう一度やる時の一般手順として残す。
